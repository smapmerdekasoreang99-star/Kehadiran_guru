import { supabaseClient, isSupabaseConfigured } from "../assets/supabase-client.js?v=20260910k";
import { demoData } from "../assets/demo-data.js?v=20260910k";
import { isUnlocked, initLockUI } from "../assets/auth-gate.js?v=20260910k";
...
function filteredRows() {
    ...
    return state.semua
        ...
        .sort((a, b) => urutanHari(a.hari) - urutanHari(b.hari) || a.jam_ke - b.jam_ke || a.kelas_id.localeCompare(b.kelas_id));
}
...
function populateModalSelects() {
    ...
    document.getElementById("fGuru").innerHTML = state.guru.map((g) => `<option value="${g.id}">${g.nama}</option>`).join("");
}
...
function openModal(id) {
    ...
    document.getElementById("fGuru").value = row ? row.guru_id : (f.guruId || state.guru[0]?.id);
